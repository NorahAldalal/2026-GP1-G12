...
if (isLoggedIn()) {
    header('Location: ' . (isAdmin() ? 'admin-home.php' : 'employee-home.php'));
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'send_code') {
    ...
    if (empty($email) || !filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        // Look up in both admin and employee tables
        $user = null;
        $name = '';

        $stmt = db()->prepare('SELECT AdminName AS name FROM `admin` WHERE Email = ?');
        $stmt->execute([$email]);
        $row = $stmt->fetch();
        if ($row) { $user = $row; $name = $row['name']; }

        if (!$user) {
            $stmt = db()->prepare('SELECT EmployeeName AS name FROM `employee` WHERE Email = ?');
            $stmt->execute([$email]);
            $row = $stmt->fetch();
            if ($row) { $user = $row; $name = $row['name']; }
        }

        // Issue cookie and send email (don't reveal if email exists)
        $code = issueResetCookie($email);
        if ($user) {
            $sent = sendResetCodeEmail($email, $name, $code);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'verify_code') {
    $entered = preg_replace('/\D/', '',
        ($_POST['d1']??'').($_POST['d2']??'').($_POST['d3']??'').($_POST['d4']??'')
    );
    $email = $_SESSION['reset_email'] ?? '';

    $result = verifyResetCode($email, $entered);
    ...
    switch ($result) {
        ...
        case 'expired':
            $_SESSION['modal_error'] = 'Code expired. Please request a new one.';
            $_SESSION['show_modal']  = false;
            unset($_SESSION['reset_email'], $_SESSION['reset_sent_at']);
            break;

        case 'max_attempts':
            $_SESSION['modal_error'] = 'Too many wrong attempts. Please request a new code.';
            $_SESSION['show_modal']  = false;
            unset($_SESSION['reset_email'], $_SESSION['reset_sent_at']);
            break;

        case 'wrong':
            $_SESSION['modal_error'] = 'Wrong code. Please try again.';
            $_SESSION['show_modal']  = true;
            $_SESSION['wrong_code']  = true;
            break;

        default: // invalid
            $_SESSION['modal_error'] = 'Invalid session. Please start over.';
            $_SESSION['show_modal']  = false;
            break;
    }
    header('Location: forgot-password.php'); exit;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'resend_code') {
    ...
    $name = $row['name'] ?? 'User';
    $sent = sendResetCodeEmail($email, $name, $code);
    echo json_encode(['ok' => $sent, 'msg' => $sent ? 'A new code has been sent!' : 'Could not send email.']);
    exit;
}
...
if (isset($_GET['resend'])) {
    clearResetCookie();
    unset($_SESSION['reset_email'], $_SESSION['reset_sent_at'], $_SESSION['show_modal'],
          $_SESSION['modal_error'], $_SESSION['wrong_code'], $_SESSION['reset_verified']);
    header('Location: forgot-password.php'); exit;
}
