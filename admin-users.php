...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['action']) && $_POST['action']==='create') {
    ...
    if (empty($username)||empty($email)||empty($password)||!$areaId) {
        ...
        $chk = db()->prepare('SELECT EmployeeID FROM `employee` WHERE Email=?');
        $chk->execute([$email]);
        if ($chk->fetch()) {
            $msg = 'This email is already registered.'; $msgType = 'error';
        } else {
            $hashed = password_hash($password, PASSWORD_BCRYPT);
            db()->prepare('INSERT INTO `employee` (EmployeeCode,EmployeeName,Email,Password,AreaID,AdminID) VALUES (?,?,?,?,?,?)')
               ->execute([$empCode ?: null, $username, $email, $hashed, $areaId, $adminId]);
...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['action']) && $_POST['action']==='update') {
    ...
    if (empty($username)||empty($email)||!$areaId) {
        ...
        $chk = db()->prepare('SELECT EmployeeID FROM `employee` WHERE Email=? AND EmployeeID!=?');
        $chk->execute([$email, $uid]);
        if ($chk->fetch()) {
            $msg = 'That email is already used by another account.'; $msgType = 'error';
        } else {
            if ($newPass) {
                db()->prepare('UPDATE `employee` SET EmployeeCode=?,EmployeeName=?,Email=?,Password=?,AreaID=? WHERE EmployeeID=? AND AdminID=?')
                   ->execute([$empCode?:null, $username, $email, password_hash($newPass,PASSWORD_BCRYPT), $areaId, $uid, $adminId]);
            } else {
                db()->prepare('UPDATE `employee` SET EmployeeCode=?,EmployeeName=?,Email=?,AreaID=? WHERE EmployeeID=? AND AdminID=?')
...
if ($_SERVER['REQUEST_METHOD']==='POST' && isset($_POST['action']) && $_POST['action']==='delete') {
    $uid = (int)$_POST['user_id'];
    db()->prepare('DELETE FROM `employee` WHERE EmployeeID=? AND AdminID=?')
       ->execute([$uid, $adminId]);
    $msg = 'Employee account deleted permanently.';
}

// ── Fetch employees ───────────────────────────────────────
$employees = db()->prepare('SELECT u.EmployeeID AS UserID, u.EmployeeCode, u.EmployeeName AS UserName, u.Email, u.AreaID, u.AdminID, u.CreatedAt, a.AreaName FROM `employee` u LEFT JOIN `area` a ON u.AreaID=a.AreaID WHERE u.AdminID=? ORDER BY u.CreatedAt DESC');
