...
// ================================================================
//  SIRAJ — Employee Analytics
//  Full chart history and weekly data table for the employee's
//  assigned area.
...
body.light-mode .area-pill{background:rgba(13,27,46,.04);border-color:rgba(13,27,46,.12);}
</style>
</head>
...
<div class="an-page">
  ...
  <div class="an-notice">
    ...
    <div><p><strong>No weekly data yet for your area.</strong><br>Analytics are generated automatically by the weekly aggregation script. Check back after the next scheduled run, or ask your administrator to run <code>cron/weekly-aggregate.php</code>.</p></div>
  </div>
  <?php endif; ?>

  <!-- ── Lamp stat row (always visible) ──────────────────── -->
...
fetch(`api/chart-data.php?area_id=${AID}&weeks=all`)
  ...
  .then(data => {
    if (data.error === 'table_missing') {
      ['line-loading','bar-loading','compare-loading'].forEach(id =>
        showChartError(id, 'Run <strong>weekly_reading.sql</strong> first.'));
      document.getElementById('data-tbody').innerHTML =
        '<tr><td colspan="5" class="no-data-msg">Import <strong>weekly_reading.sql</strong> in phpMyAdmin first.</td></tr>';
      return;
    }

    if (!data.labels || data.labels.length === 0) {
      const msg = 'No weekly data yet.<br><small>Run <code>cron/weekly-aggregate.php</code> to generate data.</small>';
      ['line-loading','bar-loading','compare-loading'].forEach(id => showChartError(id, msg));
      document.getElementById('data-tbody').innerHTML =
        '<tr><td colspan="5" class="no-data-msg">No data yet. Run the weekly aggregation script.</td></tr>';
      return;
    }
    ...
    new Chart(barCanvas, {
      type: 'bar',
      data: { labels: data.labels.slice(-4), datasets: [{ label: 'Avg Ambient', data: data.ambient.slice(-4), backgroundColor: c.bar, borderColor: '#4A90B8', borderWidth: 1 }] },
      options: {
        responsive: true, maintainAspectRatio: false,
    ...
    new Chart(compareCanvas, {
      ...
      data: {
        ...
        datasets: [
          { label: 'Ambient', data: data.ambient.slice(-8), backgroundColor: 'rgba(126,200,227,.6)', borderColor: c.line1, borderWidth: 1 },
          { label: 'Lux',     data: data.lux.slice(-8),     backgroundColor: 'rgba(74,144,184,.45)',  borderColor: c.line2, borderWidth: 1 },
        ]
      },
      options: {
        responsive: true, maintainAspectRatio: false,
  ...
  .catch(() => {
    const errMsg = 'Could not load. Make sure <strong>weekly_reading.sql</strong> is imported.';
    ['line-loading','bar-loading','compare-loading'].forEach(id => showChartError(id, errMsg));
    document.getElementById('data-tbody').innerHTML =
      '<tr><td colspan="5" class="no-data-msg">Could not load data. Make sure <strong>weekly_reading.sql</strong> is imported in phpMyAdmin.</td></tr>';
  });
</script>
</body>
</html>